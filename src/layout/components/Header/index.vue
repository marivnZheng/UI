  ...
  <div class="layout-header">
    ...
    <div class="layout-header-left"
      ...
      <div class="logo"
        ...
        <img :src="websiteConfig.logo"
             alt="" />
        <h2 v-show="!collapsed"
            class="title">{{ websiteConfig.title }}</h2>
      </div>
    ...
    <div class="layout-header-right">
      <n-select v-model:value="currentSysAccountId"
                :loading
                label-field="TgName"
      ...
      <div class="layout-header-trigger layout-header-trigger-min">
        <n-dropdown trigger="hover"
          ...
          <div class="avatar">
            <n-avatar round
                      :src="websiteConfig.admin">
              <template #icon>
                <UserOutlined />
              </template>
            </n-avatar>
            <n-divider vertical />
            <span>{{ usernamenew }}</span>
          </div>
        </n-dropdown>
      </div>
...
<script lang="ts">
import { computed, defineComponent, onBeforeMount, reactive, ref, toRefs, unref, watch } from 'vue';
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    let currentSysAccountId = ref<String>('');
    let accountList = ref([]);
    let loading = ref<Boolean>(false);

    async function getAccountList() {
      loading.value = true;
      await friendStore.getAccountList().then((res) => {
        loading.value = false;
        accountList.value = friendStore.accountList;
        currentSysAccountId.value = accountList.value[0].sysAccountId;
      });
    }
    ...
    watch(
      () => friendStore.accountList,
      (newValue) => {
        accountList.value = newValue;
        console.log(accountList.value);
      }
    );

    onBeforeMount(getAccountList);
    const drawerSetting = ref();

    const state = reactive({
      username: userStore?.info?.username ?? '',
      usernamenew: localStorage.getItem('username'),
